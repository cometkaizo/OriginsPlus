...
public class ActionOnItemRightClickAirProperty extends EventInterceptProperty {
    protected ActionOnItemRightClickAirProperty(String name, BiConsumerMap<Event, Origin> map) {
        super(name, map);
    }

    public static class Builder {
        protected String name = "Action On Item Right Click Air";
        ...
        public Builder withPlayerSensitiveAction(Class<? extends Item> type, Consumer<PlayerInteractEvent.RightClickEmpty> task) {
            return withPlayerSensitiveAction(type, (event, origin) -> task.accept(event));
        }
        public Builder withPlayerSensitiveAction(Class<? extends Item> type, BiConsumer<PlayerInteractEvent.RightClickEmpty, Origin> task) {
        ...
        public Builder withActionForEvent(Class<? extends Item> type, Consumer<PlayerInteractEvent.RightClickEmpty> task) {
            return withActionForEvent(type, (event, origin) -> task.accept(event));
        }
        public Builder withActionForEvent(Class<? extends Item> type, BiConsumer<PlayerInteractEvent.RightClickEmpty, Origin> task) {
            this.map.put(PlayerInteractEvent.RightClickEmpty.class, (event, origin) -> {
        ...
        public Builder withPlayerSensitiveAction(Item item, Consumer<PlayerInteractEvent.RightClickEmpty> task) {
            return withPlayerSensitiveAction(item, (event, origin) -> task.accept(event));
        }
        public Builder withPlayerSensitiveAction(Item item, BiConsumer<PlayerInteractEvent.RightClickEmpty, Origin> task) {
        ...
        public Builder withActionForEvent(Item item, Consumer<PlayerInteractEvent.RightClickEmpty> task) {
            return withActionForEvent(item, (event, origin) -> task.accept(event));
        }
        public Builder withActionForEvent(Item requiredItem, BiConsumer<PlayerInteractEvent.RightClickEmpty, Origin> task) {
            this.map.put(PlayerInteractEvent.RightClickEmpty.class, (event, origin) -> {
                if (event == null) return;
                if (event.isCanceled()) return;
        ...
        public ActionOnItemRightClickAirProperty build() {
            return new ActionOnItemRightClickAirProperty(name, map);
        }
    }
}
