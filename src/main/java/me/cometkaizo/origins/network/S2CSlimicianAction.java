...
public class S2CSlimicianAction {

    public static final Logger LOGGER = LogManager.getLogger();
    public final int playerId;
    private final SlimicianOriginType.Action action;

    public S2CSlimicianAction(PlayerEntity player, SlimicianOriginType.Action action) {
        this.playerId = player.getEntityId();
        this.action = action;
    }

    public S2CSlimicianAction(PacketBuffer buffer) {
        playerId = buffer.readInt();
        this.action = buffer.readEnumValue(SlimicianOriginType.Action.class);
    }
